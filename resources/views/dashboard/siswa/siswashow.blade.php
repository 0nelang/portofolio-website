...
@section('title', 'Siswa ' . $siswa->name)
@section('content')
    <style>
        .gambar {
            width: 200px;
            ...
            <div class="col-lg-4">
                <div class="card mb-4 shadow">
                    <div class="card-body" style="text-align: center">
                        <div class="gambar m-auto" style="background-image: url('{{ asset('storage/' . $siswa->foto) }}')">
                        </div>
                    ...
                    <div class="card-body">
                        ...
                        @if ($kontak->first() != null)
                            @foreach ($kontak as $index => $s)
                                @if ($index == $last)
                                    <a href="{{ $s->deskripsi }}">{{ $s->jeniskontak->jenis }} {{ $s->deskripsi }}</a>
                                @else
                                    <a href="{{ $s->deskripsi }}">{{ $s->jeniskontak->jenis }}, {{ $s->deskripsi }}</a>
                                @endif
                            @endforeach
                        @else
                            Belum ada Kontak,<a href="{{ route('kontak.create') }}">Silahkan buat Kontak</a>
                        @endif
                ...
                <div class="card shadow mb-4">
                    ...
                    <div class="card-body">
                        ...
                        @if ($project->first() != null)
                            ...
                            @endforeach
                        @else
                        Belum ada Project,<a href="{{ route('project.create') }}">Silahkan buat Project</a>
                        @endif
                    </div> {{-- card body --}}
                </div> {{-- card --}}
            </div> {{-- col-8 --}}
